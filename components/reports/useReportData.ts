...
import { ccOf, ccOfIssue, countsAsIssued, csvEsc, csvOf, fmtDate, fyStart, issueCost, label, longLabel, money, monthLabel, onhand, orderTotal, prevMonth, setsCap, shiftMonth, signedInt, signedMoney, staffName } from "@/lib/compute";
...
export function useReportData(month: string) {
  ...
  const R = useMemo(() => {
    ...
    // Attribution comes from the issue itself (ccOfIssue), not from where the wearer works today:
    // somebody moving ward must not restate what their old ward was already charged. The department
    // half of the key is a label only — there is no frozen department — so it still reads the record.
    const ccKey = (i: (typeof mIssues)[number]) => { const st = staffById[i.staffId]; return st ? (ccOfIssue(s, i, st) || "—") + "|" + (st.dept || "Unknown") : "—|Unknown"; };
  ...
  /** The column head for where somebody works, in the facility's own word. */
  const teamHead = cap(s.settings.terms.team);
  ...
  const csv = {
    ...
    approvals: () => downloadCsv(`threadcount-approvals-outstanding-${s.today}.csv`, csvOf(["Staff", teamHead, "Approved by", "Date", "Sets approved", "Collected", "Remaining"], R.apprRows.map((r) => [r.who, r.dept, r.by, r.date, r.sets, r.used, r.rem]))),
  ...
  const print = {
    ...
    approvals: () => printDoc(`Uncollected manager's approvals — as at ${fmtDate(s.today)}`, meta, [{ h: `${R.apprTot} sets outstanding`, html: tbl([C("Staff"), C(teamHead), C("Approved by"), C("Date"), C("Sets", true), C("Collected", true), C("Remaining", true)], R.apprRows.map((r) => [r.who, r.dept, r.by, fmtDate(r.date), r.sets, r.used, r.rem])) }]),
  ...
  return { R, month, mLbl, meta, jnTotItems, jnTot, csv, print, printEomPack, teamHead };
}
