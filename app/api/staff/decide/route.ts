...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (e.status === 403) {
      // The request still names its facility even when the manager or wearer is off the register,
      // so the refusal can point at the store by the facility's own word for it.
      const fac = await prisma.request.findUnique({ where: { id: claim.rid }, select: { facility: { select: { terms: true } } } });
      return NextResponse.json({ error: `That link is no longer valid — ask the ${resolveTerms(fac?.facility.terms).store}.` }, { status: 403 });
    }
