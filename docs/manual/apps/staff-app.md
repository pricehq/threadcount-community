...
screen: /my
role: Staff
keywords: staff app, details and access, wearer, nurse, self-service, code, slip, activation, sign in, kit, request, waitlist, damage, kit check, manager, approve, delivery round, round, team desk, ward round, remove access
---

The staff app shows one staff member their own uniform record, and lets them ask the uniform store for what they need. It is at `/my` in any browser, and on Google Play as **ThreadCount Staff**, which opens the same screens. There is no iOS app.
...
A forgotten password cannot be reset by email: the coordinator removes access and gives a new code.
...
The bar at the bottom has `Home`, `My kit`, `Orders` and `Messages`. A fifth, `Team`, appears for a manager, for anybody on the team desk with a team recorded, and for anybody a request is waiting on.

- **Home** shows the request furthest along with its collection code once ready, how many garments they hold, and shortcuts.
- **My kit** lists what the uniform store has recorded against them. `This isn't right` raises a query. `Slips` shows each slip the counter sent them: the date, the garments and sizes, and their signature.
...
- **Kit check** is open only while the uniform store is running one; Home prompts for it, and they confirm how many of each they still have.
- **Orders** lists their open and finished requests, each with a message thread.
- **Messages** lists every thread with the uniform store, newest first with the order each belongs to, and a `Start one` list of requests nobody has asked about yet.
...
On Android, the app can tell somebody when their request is approved or declined, when a bag is ready to collect, when it goes out on the delivery round, when a kit check opens, and — for an approver — that something is waiting on them. Each of those is a switch on their `Account` screen.
...
They cannot delete their own account; only the coordinator can.
...
- **Team.** Who on their team holds what, with each person's set count.
- **Raise for your team.** A request typed for someone who reports to them; it goes to the manager's own manager.

A person with `On the team desk` ticked in the `Staff app` panel, and a team recorded, also gets `Round`, where bags delivered to their team are signed for. Whoever signs is named on the order. See [Delivery rounds](/docs/counter/delivery-rounds).
