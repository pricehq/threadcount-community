...
A manager can raise a request in the staff app, under `Team ▸ Raise`, for the people who report to them. `Team ▸ Ward` lists who on their team holds what. Nobody approves a request they raised for somebody else, so it goes to the raiser's own manager instead. If there is nobody above, or the raiser is their own manager, it is created with no approver and waits in the request queue under `Needs an approver`.
