    ...
    case "handback.commit": {
      ...
      await lockedTx(fid, async (tx) => {
        ...
        for (const l of lines) {
          ...
          await tx.issue.create({ data: { facilityId: fid, date: today, staffId: staff.id, itemId: it.id, sizeIndex: l.swapSi, qty: 1, cond: row.preloved ? "Pre-loved" : "New", cost: row.preloved ? 0 : it.cost, preloved: row.preloved, offGroup: row.offGroup, offStyle: row.offStyle } });
