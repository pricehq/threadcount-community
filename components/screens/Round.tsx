...
import { cap, plural } from "@/lib/terms";
...
export default function RoundScreen({ ward, toSign, unclaimed, signedToday, raised }: {
  ...
  return (
    ...
    >
      <div style={{ padding: "4px 16px 0" }}><Kicker>{cap(terms.round)}</Kicker></div>
      ...
      {unclaimed.length > 0 && (
        <>
          <Band tone="attention" label={`Unclaimed from earlier ${plural(terms.round)}`} />
      ...
      {nothingAtAll ? (
        <div style={{ padding: "0 16px" }}>
          <MEmpty title={`Nothing on the ${terms.round} for ${ward || `your ${terms.team}`} right now.`} />
        </div>
      ) : (
        <>
          {toSign.map((b) => (
      ...
      {raised.length > 0 && (
        <>
          ...
          <div style={{ display: "grid", gap: 2, padding: "12px 0" }}>
            {raised.map((r) => {
              const st = statusText(r, { mine: false, first: r.subjectName?.split(" ")[0], terms });
              return (
