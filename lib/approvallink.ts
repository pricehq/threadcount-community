...
function staffFooter(facility: string) {
  return { facility, links: [] as [string, string][] };
}
...
export function approvalEmail(opts: {
  ...
  /** The facility's own words; the healthcare-free defaults when a caller has none. */
  terms?: Terms;
}) {
  ...
  const asked = raisedBy
    ? `${raisedBy} has raised a uniform request for ${opts.subjectName}, and it needs your approval before the ${t.store} can act on it.`
    : `${opts.subjectName} has asked for uniform and needs your approval before the ${t.store} can act on it.`;
...
export function decisionEmail(opts: {
  ...
  const opening = partly
    ? `${opts.managerName} approved part of your request. The approved garments are with the ${t.store} now; the rest are below, with the reason.`
    : opts.approved
      ? `${opts.managerName} approved your request. It's with the ${t.store} now.`
      : opts.reason
  ...
  const text = [
    ...
    opening,
    detail,
    opts.approved ? `You'll hear again when it's ready to collect or on its way to your ${t.team}.` : null,
    `${opts.facility} · ThreadCount`,
  ].filter((l): l is string => l !== null).join("\n\n");
  const { html } = layout({
    ...
    closing: opts.approved ? [`You'll hear again when it's ready to collect or on its way to your ${t.team}.`] : [],
    footer: staffFooter(opts.facility),
  });
  return { subject, text, html };
}
...
export function readyEmail(o: { first: string; code: string; lines: readonly NoteLine[]; collectCode?: string | null; holdUntil?: string | null; foot: string; terms?: Terms }) {
  const t = o.terms ?? DEFAULT_TERMS;
  const subject = "Ready to collect";
  const ready = `Request ${o.code} is ready at the ${t.store}.`;
  const held = [
    o.collectCode ? `Collection code: ${o.collectCode}` : null,
    o.holdUntil ? `Held until ${o.holdUntil}.` : null,
  ].filter((l): l is string => l !== null).join("\n");
  const text = [`Hi ${o.first},`, ready, garmentBlock(o.lines), held || null, o.foot]
    .filter((l): l is string => l !== null).join("\n\n");
  const { html } = layout({
    eyebrow: "Your uniform", title: subject, preheader: ready,
    intro: [`Hi ${o.first},`, ready],
    ...
    footer: { facility: o.foot.replace(/ · ThreadCount$/, "") },
  });
  return { subject, text, html };
}

/** The bag is on today's round. */
export function roundEmail(o: { first: string; code: string; lines: readonly NoteLine[]; dept?: string | null; foot: string; terms?: Terms }) {
  const t = o.terms ?? DEFAULT_TERMS;
  const subject = `On the ${t.round}`;
  const where = `Request ${o.code} is on today's ${t.round} to ${o.dept || `your ${t.team}`}.`;
