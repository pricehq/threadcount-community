...
| `Phone`, `Staff group`, `Department` | The group decides the person's route (see [groups and routes](/docs/people/groups-and-routes)). The department is their team, shown as `Team` on the record, and decides the cost centre unless an override is set. |
...
The manager and `On the team desk` are set on the record's `Details & access` tab.
...
- At most 20,000 rows at a time. There is no limit on how many records the register holds.

An import adds no group names to `Settings › Issuing rules`. The template is on [CSV templates](/docs/reference/csv-templates).

## Filter and export

The search box, `Name, number or team`, sits beside a `Group` select and a `Show` select. `Show` offers `Everyone`, `Missing something`, `No approver`, `No FTE`, `No sizes`, `No staff app`, `No uniform style`, `Receipts to sign` and `Over the ceiling`; every option but `Everyone` shows its count in brackets. When anyone is inactive, a segment switches between `Active` and `Include inactive`.
...
`Export CSV` downloads the rows on screen. The staff import reads its headers back, so a team's list can go to its manager, come back with `Manager number` filled in, and be imported again. `Approver name (reference only)` is ignored on import. Notes and start dates are left out of the file.
