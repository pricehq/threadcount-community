...
import { cap, plural as plurals } from "@/lib/terms";
...
export default function RoundsPage() {
  ...
  return (
    <section>
      <PageHead title={cap(plurals(terms.round))} sub={<span className="tc-mono">{plural(bags, "bag")} · {plural(sheet.length, terms.team, terms.teams)} · {plural(garments, "garment")}</span>} />
      <LiveRegion msg={msg} style={{ marginTop: 16, fontSize: 13, fontWeight: 600 }} />
      {sheet.length > 1 && (
        <div style={{ marginTop: 16, overflowX: "auto" }}>
          <Seg label={cap(terms.team)} opts={opts} value={current} onChange={choose} labels={labels} counts={counts} />
        </div>
      )}
