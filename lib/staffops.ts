...
import { cap, resolveTerms, type Terms } from "./terms";
...
function offGroupMessage(off: readonly { item: string; groups: string[] }[], who: { first: string; group: string }, self: boolean, t: Terms): string {
  const each = off.map((i) => `${i.item} is for ${groupsLabel(i.groups)} only`).join("; ");
  const g = (who.group || "").trim();
  const tail = self
    ? g ? `You're in ${g} — ask the ${t.store} if you need ${off.length === 1 ? "it" : "them"}.` : `You have no staff group recorded — ask the ${t.store}.`
    : g ? `${who.first} is in ${g}.` : `${who.first} has no staff group recorded — ask the ${t.store}.`;
  return `${each}. ${tail}`;
}
...
function offStyleMessage(off: readonly { item: string; gender: string }[], who: { first: string; uniformStyle: string }, self: boolean, t: Terms): string {
  const each = off.map((i) => `${i.item} is the ${genderLabel(i.gender)} cut`).join("; ");
  const tail = self
    ? `You're set to ${who.uniformStyle} — ask the ${t.store} if you need ${off.length === 1 ? "it" : "them"}.`
...
export async function decideRequest(d: {
  ...
  const r = await prisma.request.findFirst({
    ...
    include: {
      ...
      facility: { select: { name: true, terms: true } },
    },
  });
  if (!r) throw new StaffOpError("No such request", 404);
  const t = resolveTerms(r.facility.terms);
  ...
  if (!mgr) throw new StaffOpError(`You're no longer recorded as a manager here — ask the ${t.store}.`, 403);
  ...
  if (r.raisedByStaffId && r.raisedByStaffId === d.managerId && !selfApproval) {
    throw new StaffOpError(`You raised this request, so somebody else has to approve it — ask the ${t.store} to re-address it.`, 403);
  }
  ...
  if (!r.lines.length) throw new StaffOpError(`That request has no garments on it — ask the ${t.store} to raise it again.`);
  ...
  const plain = yes === decisions.length ? `${cap(t.team)} manager` : shared || refusalList;
  ...
  const em = decisionEmail({
    ...
    facility: r.facility.name, terms: t,
  });
...
export async function runStaffOp(sess: StaffSession, op: string, p: Record<string, unknown>, internal: Internal = {}): Promise<unknown> {
  ...
  const me = await prisma.staff.findUniqueOrThrow({
    ...
    select: {
      ...
      facility: { select: { id: true, name: true, timezone: true, terms: true } },
    },
  });
  const myName = `${me.first} ${me.last}`.trim();
  /** The facility's own words for its teams, store and rounds, for every message below. */
  const t = resolveTerms(me.facility.terms);
  ...
  switch (op) {
    ...
    case "request.create": {
      ...
      if (!subject.managerId) {
        throw new StaffOpError(
          subject.id === me.id
            ? `Your manager isn't set yet — the ${t.store} has to record who approves your requests.`
            : `${subject.first} has no manager recorded, so there is nobody to approve this. Ask the ${t.store} to set one.`,
        );
      }
      ...
      if (!manager) throw new StaffOpError(`The recorded manager is no longer on the register. Ask the ${t.store}.`);
      ...
      if (other && manager.id === me.id) {
        ...
        escalation = above
          ...
          : `Raised by ${myName}, who approves ${subject.first}'s requests — the ${t.store} will address it`;
        manager = above;
      }
      ...
      if (offGroup.length) throw new StaffOpError(offGroupMessage(offGroup, subject, subject.id === me.id, t));
      ...
      const offStyle = await offStyleGarments(fid, subject.uniformStyle, lines.map((l) => l.itemId),
        internal.replacing ? { staffId: subject.id, itemId: internal.replacing } : undefined);
      if (offStyle.length) throw new StaffOpError(offStyleMessage(offStyle, subject, subject.id === me.id, t));
      ...
      if (manager) {
        const em = approvalEmail({
          ...
          facility: me.facility.name, terms: t,
        });
    ...
    case "round.sign": {
      ...
      if (!me.dept || !bagWard || bagWard !== me.dept) throw new StaffOpError(`That bag is for another ${t.team}`, 403);
      ...
      try {
        await handOverRequestStock(fid, facilityToday(me.facility.timezone), { subjectId: r.subjectId, lines: r.lines }, async (tx) => {
          const moved = await tx.request.updateMany({
            ...
            data: { status: "delivered", signerName: myName, signerRole: me.wardDesk ? `${t.team} clerk` : (me.dept || t.team), signedAt: new Date() },
          });
          if (moved.count !== 1) throw new StaffOpError("Someone has already signed for that bag");
          await event(tx, r.id, `Delivered to ${me.dept || `the ${t.team}`}`, `Signed by ${myName}`, myName);
    ...
    case "round.claim": {
      ...
      if (claimed.count === 1) {
        await event(prisma, r.id, `Collected from the ${t.team}`, r.subject.id === me.id ? "" : `Marked by ${myName}`, r.subject.id === me.id ? "You" : myName);
      }
      return { ok: true };
    }
    ...
    case "damage.report": {
      ...
      if (p.replace && cat?.archived) {
        replacementNote = `That garment has been discontinued, so a replacement can't be asked for here. Your report has gone to the ${t.store}, who will sort out what you get instead.`;
      } else if (p.replace) {
    ...
    case "waitlist.join": {
      ...
      if (offGroup.length) throw new StaffOpError(offGroupMessage(offGroup, me, true, t));
      ...
      if (offStyle.length) throw new StaffOpError(offStyleMessage(offStyle, me, true, t));
    ...
    case "waitlist.accept": {
      ...
      if (holdExpired(w.offeredAt)) {
        throw new StaffOpError(`That was held for ${WAITLIST_HOLD_HOURS} hours and the hold has run out. You're still on the list — ask the ${t.store} if it's still there.`);
      }
      ...
      if (offered?.archived) {
        throw new StaffOpError(`That garment has been discontinued since you joined the list, so it can't be requested here. Ask the ${t.store} — they're holding it for you at the counter.`);
      }
