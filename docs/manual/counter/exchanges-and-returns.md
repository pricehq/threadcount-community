...
A pre-loved issue costs the team nothing and never comes off a manager's approval. It does count towards the ceiling. `Reports`, on its `People` view, shows `Pool today`. A count can cover the pool on its own ([Stocktakes](/docs/stock/stocktakes)).
