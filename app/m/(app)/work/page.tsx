...
import { plural as pluralTerm } from "@/lib/terms";
...
function Rounds() {
  ...
  if (!wards.length) return <MEmpty title={`No ${pluralTerm(s.settings.terms.round)} today`} sub={`Delivered ${pluralTerm(s.settings.terms.round)} are in History on the desktop.`} />;
  return (
    <>
      {wards.map((w) => {
