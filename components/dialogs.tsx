...
import { DEFAULT_TERMS, cap, titleCase, type Terms } from "@/lib/terms";

/** The name a slip prints when the facility has none: the store, in the facility's own word. */
const slipName = (settings: { facility: string; terms?: Terms }) => settings.facility || titleCase((settings.terms ?? DEFAULT_TERMS).store);
  ...
  return (
    <Dialog title="New order" width={640} onClose={onClose} foot={<>
      ...
      <div className="tc-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-4)", marginTop: "var(--space-4)" }}>
        <Field label="Order for">{(c) => <select {...c} className="input" value={orderFor} onChange={(e) => setOrderFor(e.target.value as "Stock" | "Staff Member")}><option value="Stock">{`Stock (${s.settings.terms.store})`}</option><option value="Staff Member">Staff member</option></select>}</Field>
    ...
    <Dialog title={staff ? "Edit staff member" : "Add staff member"} width={560} onClose={onClose} foot={<>
      ...
      <div className="tc-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-3)", marginTop: "var(--space-4)" }}>
        ...
        <Field label="Yearly report figure (garments)"
          ...
          {(c) => (nursing ? <input {...c} className="input" value="Not measured on the FTE table" disabled /> : <input {...c} className="input" value={f.ent} onChange={(e) => set("ent", e.target.value.replace(/[^0-9]/g, ""))} placeholder={`Default ${s.settings.defaultEntitlement}`} />)}
        </Field>
        ...
        <Field label={cap(s.settings.terms.team)}>
          {(c) => (
...
export function printCreditSlip(s: { settings: { facility: string; coordinator: string; terms?: Terms }; today: string }, st: StaffRec, ap: ApprovalRec) {
  ...
  const body = `<div class="slip"><div class="hd"><span class="sq"></span><b>Uniform Credit — ${esc(slipName(s.settings))}</b></div><div class="big">${remaining} of ${ap.sets} sets remaining</div><div class="meta">Staff: <b>${esc(st.first + " " + st.last)}</b> (${esc(st.num)}) · ${esc(st.dept || "")}<br>Approved by <b>${esc(ap.by || "Manager")}</b> on ${esc(fmtDate(ap.date))}${ap.fte ? " · FTE " + esc(ap.fte) : ""}${ap.notes ? "<br>Notes: " + esc(ap.notes) : ""}</div><div class="ft">Bring this slip (or just your payroll number) to the ${esc((s.settings.terms ?? DEFAULT_TERMS).store)} to collect the remaining sets — no new form needed. Printed ${esc(fmtDate(s.today))}${s.settings.coordinator ? " · " + esc(s.settings.coordinator) : ""}</div></div>`;
...
export function printAccessSlip(s: { settings: { facility: string; terms?: Terms } }, st: StaffRec, code: string) {
  ...
  const body = `<div class="slip"><div class="hd"><span class="sq"></span><b>Your uniform record — ${esc(slipName(s.settings))}</b></div>`
...
export function printHandInReceipt(s: { settings: { facility: string; coordinator: string; terms?: Terms }; today: string }, st: StaffRec, h: HandInRec, byId: Record<string, Item>) {
