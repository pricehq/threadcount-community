...
function layout(o) {
  ...
  const base = siteUrl();
  const f = footer || {};
  // Just the project's name. Nothing is invoiced, so there is no entity or ABN to print; a caller
  // may still name itself in the footer if it has reason to.
  const entity = f.entity === undefined ? "ThreadCount" : f.entity;
  // No default link: there is no support site behind this software, and on a self-hosted server a
  // link to one would point at a page that does not exist. A caller may still pass its own.
  const links = f.links || [];
