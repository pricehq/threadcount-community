...
export default function AuthForm({ initialMode, next, signupsOpen, ssoError = "", sso: ssoOffered = false }: { initialMode: "login" | "signup"; next: string; signupsOpen: boolean; ssoError?: string; sso?: boolean }) {
  ...
  const signupBody = made ? (
    <div>
      ...
      <div style={{ marginTop: 20 }}>
        {[
          ...
          ["Put the apps on the room’s phones", "The counter app for the linen room, the staff app for wearers. Both sign in with this facility’s addresses."],
        ].map(([t, s], i) => (
    ...
    <form onSubmit={(e) => { e.preventDefault(); if (step2Ok) setSuStep(3); }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 20 }}>
        <Field label="Facility name">{(c) => <input {...c} className="input" autoComplete="organization" autoCapitalize="words" autoFocus value={su.facility} onChange={(e) => setSu({ ...su, facility: e.target.value, err: "" })} placeholder="e.g. St Vincent’s Private" />}</Field>
