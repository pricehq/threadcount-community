...
handedback() { printf '%s' "$1" | sed 's/.*Handed back this year//' | head -c 400; }
check "and nothing has come back off the record yet" "$(handedback "$KIT")" '>0<'
...
check "the kit screen counts it" "$(handedback "$(curl -s -b "$S" "$B/my/kit")")" '>1<'
check "another is handed in to the pool" "$(mut handin.add "{\"staffId\":\"$AID\",\"lines\":[{\"itemId\":\"$IID\",\"si\":1,\"qty\":1,\"cond\":\"Good\"}]}")" '"good":1'
check "  and it counts the same" "$(handedback "$(curl -s -b "$S" "$B/my/kit")")" '>2<'
...
check "  which nobody handed back" "$(handedback "$(curl -s -b "$S" "$B/my/kit")")" '>2<'

echo "== leaving the app"
HOME_HTML=$(curl -s -b "$S" "$B/my")
check "Home offers a way out, through Account" "$HOME_HTML" '/my/account'
check "  and Account is where you sign out" "$(curl -s -b "$S" "$B/my/account")" 'Sign out'
