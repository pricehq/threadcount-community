...
Bug reports and patches are welcome. This is a personal open-source project maintained in spare
time, written for the way a linen room actually works, so the most
...
- **A security problem:** report it privately through this repository's security advisories
  instead. Please don't open a public issue for it, so it can be fixed before it is described in
  public.
...
Contributions are accepted under the [AGPL-3.0-only](LICENSE) licence that covers the project. The
ThreadCount name and logo are not covered by it.

Your patch stays under the AGPL, and you keep the copyright in it.
