...
export default function HomeScreen({ data, held, lastItem, managerName, kitCheckOpen }: {
  ...
  const st = live ? statusText(live, { terms }) : null;
      ...
      <MBody>
        ...
        <div style={{ padding: 16 }}>
            ...
            <div style={{ padding: "26px 0" }}>
              ...
              <div style={{ fontSize: 14, color: N600, marginTop: 2, lineHeight: 1.45 }}>
                {data.hasManager && managerName
                  ...
                  : `Your manager isn’t recorded yet — the ${terms.store} has to set who approves your requests.`}
              </div>
            </div>
          )}
          ...
          {data.wardDesk && !data.ward && (
            <div style={{ marginTop: 12 }}>
              <DarkCard kicker={cap(terms.desk)} title={`No ${terms.team} on your record`} meta={`Ask the ${terms.store} to record which ${terms.team} you are on.`} />
            </div>
          )}

          {data.notice && <Notice kicker={`From the ${terms.store}`}>{data.notice}</Notice>}
        </div>
