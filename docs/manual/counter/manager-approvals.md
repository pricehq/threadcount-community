...
Without a manager, the staff app refuses to raise a request: `Your manager isn't set yet — the uniform store has to record who approves your requests.`
...
When at least 1 garment is approved the request goes to the uniform store; when none is, it is declined.
...
- A decision by the raiser is refused: `You raised this request, so somebody else has to approve it — ask the uniform store to re-address it.`
