...
Every user of the portal at `/app` is either `Admin` or `Issuer`. The server checks the role on every change, so hiding a button is not the only protection. If an Issuer attempts a change they are not allowed to make, the server refuses it with `Admin only`, whichever screen it came from.
...
A staff sign-in lasts 30 days. It is a different kind of session from a user's, so it cannot open the portal. A user's session shows nothing in the staff app either.
...
Extra views come from the register, not from a role, and they arrive as a fifth item in the bar. If someone is named as another person's manager, they also see approvals, their team's list and `Raise` for the people who report to them. If someone is marked `On the team desk` and has a team recorded, they see the delivery round for their team. Anybody a request is addressed to sees the approvals tab even when they manage nobody else.
...
If a staff member is deactivated on the register, they lose the staff app at once. When they try to sign in they are told `You're no longer on the register at this facility. Ask the uniform store.`
