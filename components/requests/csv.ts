...
export function exportRequestsCsv(s: Snapshot, f: RequestFilter, p: RequestsPayload, showing?: string) {
  ...
  if (f === "queries") {
    ...
    downloadCsv(`threadcount-record-queries-${s.today}.csv`,
      ...
      + csvOf(["Raised", "Staff no.", "Staff member", cap(t.team), "What they say is wrong"],
  ...
  if (f === "damage") {
    ...
    downloadCsv(`threadcount-damage-${s.today}.csv`,
      ...
      + csvOf(["Reported", "Staff no.", "Staff member", cap(t.team), "Garment", "Size", "Damage", "What they said", "Replacement requested", "Photo"],
  ...
  if (f === "cycles") {
    ...
    downloadCsv(`threadcount-kit-check-${s.today}.csv`,
      ...
      + csvOf(["Staff no.", "Staff member", cap(t.team), "Garment", "Size", "On record", "Confirmed", "Short", "Answered"],
        (p.shortfalls ?? []).map((x) => [x.staffNum, x.staffName, x.ward, x.item, x.size, x.onRecord, x.confirmed, x.short, when(x.at)]))
      + "\nWaiting for a size\n"
      + csvOf(["Staff no.", "Staff member", cap(t.team), "Garment", "Size", "Waiting since", "Offered", "Held until", "Hold"],
        (p.waiting ?? []).map((w) => {
  ...
  downloadCsv(`threadcount-requests-${f === "noapprover" ? "needs-an-approver" : f}-${s.today}.csv`,
    preamble([
      [`${cap(t.team)} requests`, REQUEST_FILTER_LABEL[f]],
      ["Exported", s.today],
      ["Times shown in", s.tz],
    ...
    + csvOf(["Request", "Raised", "Staff no.", "Staff member", cap(t.team), "Raised by", "Reason", "Note", "Request status", "Approver", "Approver is the wearer", "Decision summary", "Decided", "Request decline reason", "Collection code", "Garment", "Cut", "Size", "Qty", "Line decision", "Line decline reason"],
      rows.flatMap((r) => {
        const req: (string | number)[] = [
          r.code, when(r.createdAt), r.staffNum, r.staffName, r.ward, r.raisedByName, r.reason, r.note,
          statusText(r, { mine: false, terms: t }).label, r.managerName, r.managerId && r.managerId === r.staffId ? "Yes" : "",
