...
export async function POST(req: NextRequest) {
  ...
  const u = await prisma.$transaction(async (tx) => {
    ...
    // There are no plans and nothing to pay: every facility is created with everything, for good.
    // The two columns survive because the schema still carries them; nothing reads them to decide
    // what a room may do.
    const planData = { grandfathered: true, planStatus: "free" };
