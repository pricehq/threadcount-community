...
import { cap, resolveTerms } from "@/lib/terms";
...
export default async function PrintPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const title = type === "delivery" ? `Uniform ${t.team} delivery` : "Uniform ready for collection";
  ...
  const Slip = () => (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-start" }}>
      ...
      {type === "delivery" ? (
        <>
          <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "1.4fr 1.4fr 0.5fr", columnGap: 18 }}><F label={cap(t.team)} v={q.dept} /><F label={`Deliver to (location in the ${t.team})`} v={q.deliverTo} /><F label="Garments" v={q.sets} /></div>
          ...
          <div style={{ marginTop: 12, paddingTop: 8, borderTop: "2px solid #201e1d", display: "grid", gridTemplateColumns: "1fr 0.8fr 1.4fr", columnGap: 18 }}><F label="Delivered by" v={q.deliveredBy} /><F label="Date / time" v={q.dateTime} /><F label={`Received for the ${t.team} by (name + sign)`} /></div>
        </>
      ) : (
        <>
          <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr", columnGap: 18 }}><F label={cap(t.team)} v={q.dept} /><F label="Date received" v={q.dateReceived} /><F label="PO / Order no." v={q.po} /></div>
