...
2. **Your facility.** The facility name, plus two optional answers. `Kind of organisation` sets the [words](/docs/start/your-words) the screens use, and `Hospital or healthcare` also adds a starting list of staff groups. `State or territory` sets the time zone that counts and month-end are read in, and nothing else.
...
| Record | Change | Undo |
|---|---|---|
| Facility | Created with your name as coordinator, the words for your kind of organisation, any seeded staff groups and the time zone | See [Delete an account](/docs/account/delete-an-account) |
