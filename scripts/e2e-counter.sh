...
# One row per garment and size, carrying how many were signed for — not the same row repeated. The
# quantity is the one number a wearer is shown that is not their own record, and it is theirs: the
# hand-over they stood at the counter and put their name to. Read off the values, so a line that
# gained the key and lost the count would still be caught.
check "  each carrying how many were signed for" "$(echo "$SLIPS" | py 'print(sorted((l["item"], l["qty"]) for l in d[0]["lines"]))')" "\[('RN Scrub Top', 1), ('Security Shirt', 1)\]"
# The key set is pinned rather than sampled: this is the check that catches a cost, an approver or
# the issuer's name arriving on a wearer's slip because somebody widened a select. `qty` joined it
# deliberately in the staff-app redesign; nothing else may.
check "a slip carries only its id, date, garments and signature" "$(echo "$SLIPS" | py 'print(sorted(d[0]), sorted({k for l in d[0]["lines"] for k in l}))')" "\['date', 'id', 'lines', 'signed'\] \['item', 'qty', 'size'\]"
