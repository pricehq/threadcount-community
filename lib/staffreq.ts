 ...
 */
import { DEFAULT_TERMS, cap, type Terms } from "./terms";
...
export function statusText(r: {
  ...
  first?: string;
  /** The facility's words (lib/terms.ts). Omitted, the generic set. */
  terms?: Terms;
  ...
  const t = view.terms ?? DEFAULT_TERMS;
  switch (r.status as ReqStatus) {
    ...
    case "accepted":
      return { label: `Approved — with the ${t.store}`, note: "Waiting to be picked", ink: "quiet" };
    case "picking":
      return { label: "Being picked", note: `In the ${t.store}`, ink: "quiet" };
    case "ready":
      return { label: "Ready to collect", note: r.holdUntil ? `${cap(t.store)} · until ${r.holdUntil}` : cap(t.store), ink: "attention" };
    case "round":
      return { label: `On the ${t.round}`, note: r.ward ? `Arriving on ${r.ward}` : `Arriving on ${whose} ${t.team}`, ink: "attention" };
    case "collected":
      return { label: "Collected", note: "Signed off at the counter", ink: "quiet" };
    case "delivered":
      return {
        label: mine ? `Delivered to your ${t.team}` : r.ward ? `Delivered to ${r.ward}` : `Delivered to the ${t.team}`,
        note: r.signerName ? `Signed by ${r.signerName}${r.signerRole ? `, ${r.signerRole}` : ""}` : `Signed for on the ${t.team}`,
        ink: "quiet",
      };
    default:
...
export function teamTabs(
  me: { isManager: boolean; wardDesk: boolean; ward: string; terms?: Terms },
  ...
  if (me.isManager) {
    tabs.push({ href: "/my/ward", label: cap((me.terms ?? DEFAULT_TERMS).team) });
 ...
 *  they are on a desk. */
export function teamTitle(me: { isManager: boolean; wardDesk: boolean; terms?: Terms }, counts: { approvals: number }): string {
  if (me.isManager || counts.approvals > 0) return "Your team";
  return me.wardDesk ? cap((me.terms ?? DEFAULT_TERMS).desk) : "Your team";
}
