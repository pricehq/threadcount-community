...
export async function signInStaff(email: string, password: string, ip: string, ownDoor: boolean): Promise<StaffSignIn> {
  const acc = await prisma.staffAccount.findUnique({
    where: { email },
    select: { id: true, facilityId: true, passwordHash: true, staff: { select: { id: true, first: true, last: true, inactive: true } }, facility: { select: { terms: true } } },
  });
  ...
  if (acc.staff.inactive) {
    ...
    return { kind: "error", error: `You're no longer on the register at this facility. Ask the ${resolveTerms(acc.facility.terms).store}.`, status: 403 };
  }
