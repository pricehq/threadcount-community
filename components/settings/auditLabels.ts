...
/** The few names that say the facility's own words, so they are built per facility. */
const TERMED: Record<string, (t: Terms) => string> = {
  "pickup.deliver": (t) => `Delivered a bag on the ${t.round}`,
  "pickup.deliverWard": (t) => `Delivered the bags for one ${t.team}`,
  "request.round": (t) => `Put a request on the ${t.round}`,
  "notice.set": (t) => `Changed the notice for ${t.teams}`,
  "staff:round.sign": (t) => `Signed for a delivery to their ${t.team}`,
  "staff:round.claim": (t) => `Confirmed a bag was collected from the ${t.desk}`,
};

/** An op's plain-English name in this facility's words, or the op itself when it has none. */
export function auditLabel(op: string, t: Terms): string {
  return TERMED[op]?.(t) ?? LABELS[op] ?? op;
}
