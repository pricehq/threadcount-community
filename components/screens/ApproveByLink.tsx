...
type Data = {
  ...
  /** The facility's words (lib/terms.ts) — this page is outside the staff app's provider. */
  terms: Terms;
};
    ...
    return (
      <MyShell>
        ...
        <p style={lead}>
          ...
          {done.approved
            ? `${done.notified ? `${data.subjectName.split(" ")[0]} has been told, and` : `${data.subjectName.split(" ")[0]} hasn’t been emailed — it’s on their record in the app — and`} ${data.lines.length > 1 ? "all of it is" : "it’s"} with the ${data.terms.store} now.`
      ...
      <MyShell>
        ...
        <p style={lead}>
          {status === "declined"
            ...
            : `This request has already been approved and is with the ${data.terms.store}.`}
        </p>
