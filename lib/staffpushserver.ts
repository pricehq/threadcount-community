...
export function notifyText(
  kind: Kind,
  f: { id?: string; code?: string; garments?: string; ward?: string; who?: string; dueBy?: string; terms?: Terms },
): Msg {
  const t = f.terms ?? DEFAULT_TERMS;
  ...
  switch (kind) {
    ...
    case "round": return { title: `On the ${t.round}`, body: `${f.code} · ${f.ward ? `arriving at ${f.ward}` : `arriving at your ${t.team}`}`, url: order, tag };
...
async function onRound(requestId: string): Promise<number> {
  if (!pushConfigured()) return 0;
  const r = await prisma.request.findUnique({ where: { id: requestId }, include: { ...REQ, facility: { select: { terms: true } } } });
  if (!r) return 0;
  return push(await devicesFor([r.subjectId], "round"), notifyText("round", { id: r.id, code: r.code, ward: r.subject.dept, terms: resolveTerms(r.facility.terms) }));
}
