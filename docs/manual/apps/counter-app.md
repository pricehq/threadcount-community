...
Every room runs its own server, so the app has to be told which one before it can do anything. That is done once, on each phone.

The first time you open the Android app, the welcome screen shows **Sign in**, **Sign up** and a line reading `Server: … · Change`.

1. **Tap the `Server` line.** The `Which server?` screen opens.
2. **Type your facility's server address,** for example `uniforms.example.com`.
