...
export default function WaitlistScreen({ data }: { data: Data }) {
  ...
  const noticeLine = accepted
    ...
    : heldForMe
      ...
      : lapsed
        ? `Nobody took it before the hold ran out, so you are ${ordinal(data.position)} in queue again and the ${me.terms.store} can offer it a second time.`
        : joined
          ? `${ordinal(data.position)} in queue. The ${me.terms.store} tells you when one is held for you.`
