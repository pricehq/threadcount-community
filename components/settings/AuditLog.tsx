...
import { NOTABLE, auditLabel } from "./auditLabels";
...
export default function AuditLog({ scrollTo }: { scrollTo?: boolean }) {
  ...
  function exportCsv() {
    ...
    downloadCsv(`threadcount-activity-${s.today}.csv`,
      ...
      + csvOf(["When", "Who", "What", "Record", "Record id"], events.map((e) => [stamp(e.at), e.who, auditLabel(e.op, s.settings.terms), recordText(recordParts(ix, e.target)), e.target])));
  }
              ...
              return (
                <tr key={e.id}>
                  ...
                  <td style={{ fontWeight: notable ? 700 : 400, color: notable ? "var(--color-accent-700)" : undefined }}>
                    ...
                    {auditLabel(e.op, s.settings.terms)}
                  </td>
