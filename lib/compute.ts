...
export type Snapshot = {
  ...
  /** The plan, worked out from the facility's plan columns by lib/plan.ts. `live` is the platform
   *  switch; until it is on, the screens show nothing about plans at all. */
  plan: {
    code: "hosted_small" | "hosted_facility" | "health_service" | "private";
    label: string;
    state: "grandfathered" | "free" | "trial" | "active" | "grace" | "read_only";
    readOnly: boolean;
    maxStaff: number | null; staff: number; backupDays: number;
    endsAt: string | null; daysLeft: number | null;
    graceEndsAt: string | null; graceDaysLeft: number | null;
    grandfathered: boolean;
    /** Admins only; blank for everyone else. */
    billingEmail: string;
    live: boolean;
    /** Stripe is configured on this instance; a subscription exists for this facility. */
    cardsOn: boolean;
    card: boolean;
    /** From the checkout: who the invoice is made out to (admins only), where, and month or year. */
    billingLegalName: string;
    billingCountry: string;
    billingCadence: string;
    /** The health service this facility belongs to, if any. */
    org: { name: string } | null;
    /** The invoicing entity and ABN, when configured on the instance. */
    invoicer: string;
  };
  settings: Settings;
  catalog: Item[];
...
export function ccBudgetNote(s: Snapshot, byId: Record<string, Item>, staffById: Record<string, StaffRec>, code: string, suffix = ""): string {
  ...
  for (const i of s.issues) { if (!countsAsIssued(i) || i.preloved || i.date.slice(0, 7) !== mon) continue; const st = staffById[i.staffId]; if (st && ccOf(s, st) === code) iss += i.qty * issueCost(i, byId); }
  let onOrd = 0;
