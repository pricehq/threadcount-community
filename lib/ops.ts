...
import { checkTerms, resolveTerms, titleCase, type Terms } from "./terms";
 ...
 *  and a mystery. */
const DEMO_FIXED: Record<string, string | ((t: Terms) => string)> = {
  logoData: "the slip logo",
  // Every screen of the demo reads these, so one visitor renaming the wards would rename them under
  // everyone else looking at it.
  terms: "the words the screens use",
  ...
  coordinatorEmail: (t) => `the ${t.store}'s e-mail address`,
  coordinatorPhone: (t) => `the ${t.store}'s phone number`,
...
export async function runOp(user: SessionUser, op: string, p: any): Promise<unknown> {
  ...
  const fac = await prisma.facility.findUniqueOrThrow({ where: { id: fid }, select: { name: true, timezone: true, terms: true } });
  /** The facility's own words for its teams, store and rounds, for every message below. */
  const t = resolveTerms(fac.terms);
  ...
  const signOff = () => `${fac.name || titleCase(t.store)} · ThreadCount`;
  p = p || {};
  ...
  switch (op) {
    ...
    case "settings.update": {
      ...
      if (user.isDemo) {
        ...
        if (fixed) throw new OpError(`The demo is one facility shared with everyone looking at it right now, so ${((f) => (typeof f === "function" ? f(t) : f))(DEMO_FIXED[fixed])} is fixed here. The rest of this screen is yours to change.`, 403);
      }
      ...
      // All five words at once, never one: they are read as a set, and a half-renamed facility
      // would say "crew" on one screen and "ward" on the next.
      if (p.terms !== undefined) {
        try { data.terms = checkTerms(p.terms); }
        catch (e) { throw new OpError((e as Error).message); }
      }
      ...
      if (p.coordinatorPhone !== undefined) {
        ...
        if (ph && !/\d/.test(ph)) throw new OpError(`A phone number needs some digits in it — leave it blank if the ${t.store} doesn't take calls.`);
      ...
      if (p.timezone !== undefined) {
        ...
        if (!z) throw new OpError(`Pick the time zone the ${t.store} is in`);
    ...
    case "catalog.removeSize": {
      ...
      if (reqs) blocked.push(`a ${t.team} request`);
    ...
    case "request.collected": {
      ...
      let label = "", meta = "";
      if (to === "picking") { label = "Being picked"; meta = str(p.where, 60) || `In the ${t.store}`; }
      if (to === "ready") {
      ...
      if (to === "round") {
        ...
        if (!r.subject.dept) throw new OpError(`${r.subject.first} has no ${t.team} recorded, so there is no ${t.round} to send this on. Hold it at the counter, or record their ${t.team} on the staff register first.`);
        ...
        if (!signers) throw new OpError(`Nobody on ${r.subject.dept} can sign for a ${t.round} bag — that needs somebody with the ${t.desk} flag and their own staff-app account. Hold it at the counter instead, or set the flag on their staff record first.`);
      ...
      if (acct?.email && transactionalConfigured() && (to === "ready" || to === "round")) {
        ...
        const em = to === "ready"
          ? readyEmail({ first: r.subject.first, code: r.code, lines: noteLines, collectCode: fresh.collectCode, holdUntil: fresh.holdUntil, foot: signOff(), terms: t })
          : roundEmail({ first: r.subject.first, code: r.code, lines: noteLines, dept: r.subject.dept, foot: signOff(), terms: t });
    ...
    case "request.reassign": {
      ...
      if (op === "request.withdraw") {
        const why = str(p.reason, 120).trim() || `Withdrawn by the ${t.store}`;
        ...
        if (subjAccount?.email && transactionalConfigured()) {
          const em = decisionEmail({
            ...
            lines: reqLines, facility: fac.name, terms: t,
          });
    ...
    case "me.deleteAccount": {
      ...
      if (last) {
        // Nothing to stop before deleting any more: there is no money in the product, so a facility
        // deletes on the spot. This used to cancel a card subscription first and refuse the delete
        // if it could not — which, with payments gone, would have refused every delete on a row
        // that still carried an old subscription id, pointing the user at a Settings tab that no
        // longer exists.
        //
...
export async function exportBackup(user: SessionUser) {
  ...
  return { format: "threadcount-backup-v2", exportedAt: new Date().toISOString(), photosOmitted, facility: fac, items: items.map((it) => ({ ...it, group: it.groups.length ? it.groups.join("|") : "All" })), barcodes, stock, moves, depts, suppliers, staff, approvals, alterations, issues, orders, pickups, stocktakes, handins, photos: photosOut, locations, costs, requests, waitlist, kitChecks, damage, disputes, notices, slips };
}
