...
export async function kitData(sess: StaffSession) {
  ...
  /* Signed slips sent to the staff app from the counter phone. Words only, the staff-app rule: the
   * garment and size once per garment handed over (no quantities), the date, and whether a
   * signature is attached (served by /api/staff/slip/[id]/sig). No cost, no counts, no approver and
   * nobody's name. */
  ...
  const slips = slipRows.map((r) => {
    const lines: { item: string; size: string }[] = [];
    for (const raw of Array.isArray(r.lines) ? r.lines : []) {
      ...
      for (let i = 0; i < n; i++) lines.push({ item: it.item, size: String(it.sizes[si] ?? si) });
    }
    return { id: r.id, date: r.date, lines, signed: !!r.sigId };
  });
