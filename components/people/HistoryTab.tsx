            ...
            <div className="tc-people-list">
              {forms.map((x) => x.kind === "approval" ? (
                ...
                <div key={`r${x.r.id}`} className="tc-people-item">
                  ...
                  <span className="grow"><b className="tc-mono">{x.r.code}</b> · {x.r.garments} {x.r.garments === 1 ? "garment" : "garments"}{x.r.reason ? ` · ${x.r.reason}` : ""} · {statusText(x.r, { mine: false, terms: s.settings.terms }).label}</span>
