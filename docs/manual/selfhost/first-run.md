...
4. **Step 2, Your facility.** The facility name, plus two optional answers. The kind of organisation sets the [words](/docs/start/your-words) the screens use, and for a hospital or healthcare service also adds a starting list of staff groups. The state sets the time zone that counts and month-end use.
...
Your server answers the Android apps as well as a browser. Each phone is told which server to use, and the apps have no default: on the app's first screen, tap the `Server` line (or `Sign in`) and enter your hostname. The app checks it over https and uses it from then on. See [the counter app](/docs/apps/counter-app).
