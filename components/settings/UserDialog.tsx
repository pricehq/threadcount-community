...
import { titleCase } from "@/lib/terms";
  ...
  return (
    <Dialog title={user ? "Edit user" : "Add user"} width={520} onClose={onClose}>
      <div className="tc-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-3)", marginTop: "var(--space-4)" }}>
        ...
        <Field label="Title">{(c) => <input {...c} className="input" value={f.title} onChange={(e) => setF({ ...f, title: e.target.value })} placeholder={`e.g. ${titleCase(s.settings.terms.store)} Assistant`} />}</Field>
